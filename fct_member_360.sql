        ...
        create or replace transient table MUSEUM_DW_PROD.GOLD.fct_member_360
        copy grants as
        (select * from (
              

WITH crm AS (
    SELECT * FROM MUSEUM_DW_PROD.SILVER.silver_sf_crm
...
LEFT JOIN email_engagement ee ON c.email = ee.email_address
              ) order by (engagement_segment, computed_membership_status)
        );
      alter  table MUSEUM_DW_PROD.GOLD.fct_member_360 cluster by (engagement_segment, computed_membership_status);
