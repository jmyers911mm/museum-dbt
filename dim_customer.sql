        ...
        create or replace transient table MUSEUM_DW_PROD.GOLD.dim_customer
        copy grants as
        (select * from (
...
WITH identifiers AS (
    SELECT
        ...
        source_system,
        source_id
    FROM MUSEUM_DW_PROD.BRONZE.raw_customer_identifiers
...
crm_match AS (
    SELECT
        ...
        c.opt_in_email
    FROM grouped g
    LEFT JOIN MUSEUM_DW_PROD.BRONZE.raw_sf_crm c
...
FROM crm_match
      ...
      alter  table MUSEUM_DW_PROD.GOLD.dim_customer cluster by (customer_id);
