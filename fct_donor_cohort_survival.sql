        ...
        create or replace transient table MUSEUM_DW_PROD.GOLD.fct_donor_cohort_survival
        copy grants as
        (select * from (
              

WITH retention AS (
    SELECT * FROM MUSEUM_DW_PROD.GOLD.fct_donor_retention
),
...
FROM survival_by_cohort
      ...
      alter  table MUSEUM_DW_PROD.GOLD.fct_donor_cohort_survival cluster by (months_since_acquisition);
